...
describe('Event Controllers', () => {
  ...
  describe('create', () => {
    it('should create a new event', async () => {
      ...
      expect(createEvent).toHaveBeenCalledWith({ name: 'Test Event' });
    });

    it('should return 400 if there is an error', async () => {
      (createEvent as jest.Mock).mockRejectedValue(new Error('Create event error'));
      ...
      expect(response.body.error).toBe('Create event error');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should return 400 if there is an error', async () => {
      (updateEvent as jest.Mock).mockRejectedValue(new Error('Update event error'));
  ...
  describe('getAll', () => {
    ...
    it('should return 400 if there is an error', async () => {
      (getAllEvents as jest.Mock).mockRejectedValue(new Error('Get all events error'));

      const response = await request(app)
        .get('/events');

      expect(response.status).toBe(400);
      expect(response.body.error).toBe('Get all events error');
    });
  });
  ...
  describe('getById', () => {
    ...
    it('should return 400 if there is an error', async () => {
      (getEventById as jest.Mock).mockRejectedValue(new Error('Get event error'));

      const response = await request(app)
        .get('/events/event-1');

      expect(response.status).toBe(400);
      expect(response.body.error).toBe('Get event error');
    });
  });
  ...
  describe('remove', () => {
    ...
    it('should return 400 if there is an error', async () => {
      (getEventById as jest.Mock).mockRejectedValue(new Error('Delete event error'));

      const response = await request(app)
        .delete('/events/event-1');

      expect(response.status).toBe(400);
      expect(response.body.error).toBe('Delete event error');
    });
  });
});